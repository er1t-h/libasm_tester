    ...
    macro_rules! test {
        ($name: ident, $to_test: expr) => {
            ...
            fn $name() {
                ...
                let ret_val = unsafe { ft_strcmp(s1.as_ptr(), s2.as_ptr()) };
                assert_eq!(ret_val, 0);
            }
        };
...
mod read {
    ...
    macro_rules! test {
        ($name: ident, $file_name: expr) => {
            ...
            fn $name() {
                let file_content =
                    std::fs::read_to_string(concat!("test_files/read/", $file_name, ".txt"))
                        .expect("Cannot read file");
                let file = File::open(concat!("test_files/read/", $file_name, ".txt"))
                    .expect("Cannot open file");
                let fd = file.as_raw_fd();
                let mut buffer = vec![0_u8; file_content.len()];
                let length = file_content.len();
                let ret_val = unsafe { ft_read(fd, buffer.as_mut_ptr() as *mut c_void, length) };
